...
#define TAG "de.appwerft.audiomanager"

using namespace v8;

static Persistent<Object> bindingCache;

static void Audiofocus_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Audiofocus.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = titanium::bindings::AudiofocusBindings::lookupGeneratedInit(
...
static void Audiofocus_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Audiofocus_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Audiofocus_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			titanium::bindings::AudiofocusBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AudiofocusBinding = {
	"de.appwerft.audiomanager",
	Audiofocus_init,
	Audiofocus_dispose
};
...
Java_de_appwerft_audiomanager_AudiofocusBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("de.appwerft.audiomanager", &AudiofocusBinding);
	titanium::KrollBindings::addExternalLookup(&(titanium::bindings::AudiofocusBindings::lookupGeneratedInit));
}
